...
void Widget::addJob(int jSize) {
    ...
    if(!found)
        ...
        // 修改内存分区链表
        mem_Block *temp_mem = head_mem -> next;
        cout<<"the block id in mem is :"<< temp_mem->id<<endl;
...
void Widget::reapJob(int reapJobID) {
    ...
    if(!found)
        ...
        // 若已找到所需回收的分区
        if((temp->id == 1 && temp->next->state == 1)  // 所需回收的分区是第一块,且其后的一个分区已被占用
        ...
        || (temp->prior->state == 1 && temp->next->state == 1)){   // 所需回收的分区是中间块,且相邻分区都已被占用
            ...
            || (temp->prior->state == 1 && temp->next->state == 0)) {  // 回收中间块,且其下为空
            // 在空闲分区链中找到对应的空闲块
            ...
            || (temp->prior->state == 0 && temp->next->state == 1)){   // 回收中间块,其上为空
            // 在空闲分区链中找到对应的空闲块
            mem_Block *temp_free = head_free->next;
            ...
            temp_free->size += temp_free->next->size;
